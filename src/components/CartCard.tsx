...
import { decreasePriceCounter, deleteCartProduct, increasePriceCounter, initializeCartItem } from "@/redux/features/totalPriceSlice";
        ...
        <div className="py-4 flex justify-between items-center gap-x-6 xl:w-10/12 mx-auto px-2">
            ...
            <div className="flex flex-col xl:flex-row xl:gap-x-4 gap-y-4 items-center" >
                <div className="flex items-center border-2 w-[124px] border-black justify-around py-2 rounded-md">
                    <button
                        onClick={() => dispatch(decreasePriceCounter({
                            id: cart._id,
                            quantity: cart.quantity,
                            price: cart.price,
                            orderCount: cart.orderCount
                    ...
                    <p className="text-md px-4 font-medium border-2 border-y-0 border-black">{orderCount}</p>

                    <button
                        onClick={
                            () => dispatch(increasePriceCounter({
                                id: cart._id,
                                quantity: cart.quantity,
                                price: cart.price,
                                orderCount: cart.orderCount
                ...
                <div className="cursor-pointer hover:scale-105 duration-200" onClick={() => handleDelete(cart?._id)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 text-def hover:text-red-700">
